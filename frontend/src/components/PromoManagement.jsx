        ...
        <form onSubmit={handleSavePromo} className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <input 
              ...
              required 
              className="w-full p-2 bg-gray-900 rounded text-sm text-white uppercase border border-gray-700" 
              value={promoCode} 
          ...
          <button className="w-full bg-yellow-600 hover:bg-yellow-700 py-2 rounded text-sm font-bold text-black transition">
            {editPromoId ? 'Update Promo' : 'Simpan Promo'}
          </button>
        </form>
      </div>
      ...
      <div className="bg-gray-800 p-6 rounded-xl border border-gray-700 space-y-4">
        ...
        <div className="space-y-2 max-h-80 overflow-y-auto pr-1">
          {promos && promos.map(pr => {
            // Ambil nilai secara aman langsung dari objek pr
            const discountVal = Number(pr.discount || 0);
            const minSpendVal = Number(pr.min_spend || 0);

            return (
              <div key={pr.id || Math.random()} className="bg-gray-900 p-3 rounded-lg border border-gray-700 flex justify-between items-center">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-sm text-yellow-400 uppercase">{pr.code || 'KODE'}</span>
                  ...
                  <div className="text-xs text-gray-400 mt-1">
                    ...
                    {minSpendVal > 0 ? ` (Min: Rp ${minSpendVal.toLocaleString('id-ID')})` : ''}
                  </div>
                </div>
          ...
          {(!promos || promos.length === 0) && <p className="text-xs text-gray-500 italic">Belum ada promo.</p>}
        </div>
      </div>
    </div>
  );
